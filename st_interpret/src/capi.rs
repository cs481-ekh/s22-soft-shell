...
/// Loads a ST program from a file and returns a pointer to a program context
/// # Arguments
/// * `filename` - the name of the file to load
#[no_mangle]
pub extern "C" fn st_program_load(filename: *const c_char) -> ProgContextPointer {
    unsafe {
        let context =
            prog_handle::st_program_load(CStr::from_ptr(filename).to_str().unwrap()).unwrap();
        ProgContextPointer {
            prog_context: Box::new(context),
        }
    }
}
...
/// # Arguments
/// * `program_context` - a pointer to a program context that was generated by st_program_load
#[no_mangle]
pub extern "C" fn st_program_step(program_context: &mut ProgContextPointer) -> bool {
    prog_handle::st_program_step(&mut program_context.prog_context).unwrap()
}
...
/// * `program_context` - a pointer to a program context that was generated by st_program_load
#[no_mangle]
pub extern "C" fn st_program_run(program_context: &mut ProgContextPointer) {
    let _result = prog_handle::st_program_run(&mut program_context.prog_context);
}
...
pub extern "C" fn get_all_vars(
    program_context: &ProgContextPointer,
) -> Option<Box<VariableNameInfo>> {
    let vars = program_context.prog_context.get_all_vars();
...
pub extern "C" fn update_var(
    program_context: &mut ProgContextPointer,
    name: *const c_char,
...
pub extern "C" fn add_var(
    ...
    unsafe {
        ...
        let _result = program_context.prog_context.add_var(
...
#[repr(C)]
pub struct ProgContextPointer {
    prog_context: Box<ProgContext>,
}
